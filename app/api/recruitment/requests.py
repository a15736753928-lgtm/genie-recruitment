...
async def ai_generate(
    ...
    try:
        ...
        async def gen_job_description():
            """生成岗位说明书（结构化 JSON）"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info}, 'Prompt 1')
        ...
        async def gen_competency_model():
            """生成能力模型"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info}, 'Prompt 2')
        ...
        async def gen_resume_scoring():
            """生成简历评分规则"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info}, 'Prompt 3')
        ...
        async def gen_interview_r1():
            """生成一面维度"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info}, 'Prompt 4')
        ...
        async def gen_interview_r2():
            """生成二面维度"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info}, 'Prompt 5')
            resp = await llm_chat([{"role": "user", "content": prompt}], max_tokens=1500)
            return ("interviewDimensionsR2", extract_json_array(resp))

        async def gen_probation_framework():
            """生成试用期框架"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info, 'req_probation_goal': req.probation_goal, 'req_elimination_criteria': req.elimination_criteria}, 'Prompt 6')
            resp = await llm_chat([{"role": "user", "content": prompt}], max_tokens=1024)
            return ("probationFramework", extract_json_object(resp))

        async def gen_training_plan():
            """生成培训建议"""
            prompt = render_prompt('recruitment/requests.md', {'base_info': base_info}, 'Prompt 7')
            resp = await llm_chat([{"role": "user", "content": prompt}], max_tokens=1500)
